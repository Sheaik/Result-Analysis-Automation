...
var JAR_PATH = path.join(__dirname, './tabula-java.jar');

var parseCommandArgs = function parseCommandArgs() {
  var args = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  return Object.entries(args).reduce(function (acc, curr) {
    var _curr = _slicedToArray(curr, 2),
        key = _curr[0],
        value = _curr[1];

    var modifiedKey = "--".concat(lodash.kebabCase(key));

    if (Array.isArray(value)) {
      return acc.concat(value.reduce(function (acc, curr) {
...
function () {
  function TabulaCommand(pdfPath, commandArgs) {
    _classCallCheck(this, TabulaCommand);

    this._build(pdfPath, commandArgs);
  }

  _createClass(TabulaCommand, [{
    key: "_build",
    value: function _build(pdfPath, commandArgs) {
      var silentArgs = commandArgs && commandArgs.silent ? ['-Dorg.slf4j.simpleLogger.defaultLogLevel=off', '-Dorg.apache.commons.logging.Log=org.apache.commons.logging.impl.NoOpLog', '-Dfile.encoding=utf-8'] : [];
      this._args = [].concat(silentArgs, ['-jar', JAR_PATH], _toConsumableArray(parseCommandArgs(commandArgs)), [pdfPath]);
    }
  }, {
    key: "run",
    value: function run() {
...
function () {
  ...
  _createClass(Tabula, [{
    ...
    key: "streamSections",
    value: function streamSections(callback) {
      this.stream().map(function (data) {
